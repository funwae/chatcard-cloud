...
passkeyRouter.post('/register/options', authLimiter, async (req, res) => {
    try {
...
passkeyRouter.post('/register/verify', authLimiter, async (req, res) => {
    try {
        const { body } = req;
...
passkeyRouter.post('/login/verify', authLimiter, async (req, res) => {
    try {
        ...
        const credential = user.webauthnCredentials.find((cred) => cred.id === body.response.id);
        if (!credential) {
            challenges.delete(body.response.challenge);
...
//# sourceMappingURL=auth-passkey.js.map