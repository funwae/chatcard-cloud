...
proposalsRouter.post('/', requireAgentOrOwner, requireScope('propose'), async (req, res) => {
    try {
        if (!req.userDid) {
...
proposalsRouter.get('/', requireOwner, async (req, res) => {
    try {
        if (!req.userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
...
proposalsRouter.post('/:id/approve', requireOwner, async (req, res) => {
    try {
        if (!req.userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
...
proposalsRouter.post('/:id/reject', requireOwner, async (req, res) => {
    try {
        if (!req.userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
