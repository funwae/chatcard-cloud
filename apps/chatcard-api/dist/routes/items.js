...
itemsRouter.get('/', requireOwner, async (req, res) => {
    try {
        if (!req.userId) {
...
itemsRouter.post('/sections/:section/items', requireOwner, async (req, res) => {
    try {
        if (!req.userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
...
itemsRouter.patch('/:id', requireOwner, async (req, res) => {
    try {
        if (!req.userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
...
itemsRouter.delete('/:id', requireOwner, async (req, res) => {
    try {
        if (!req.userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
