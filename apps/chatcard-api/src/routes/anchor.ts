...
anchorRouter.post('/:mh/anchor', requireOwner, async (req: AuthRequest, res) => {
  try {
    if (!req.userId) {
    ...
    const p = provider ?? (process.env.ANCHOR_DEFAULT_PROVIDER as 'none' | 'opentimestamps' | 'evm-l2') ?? 'none';
