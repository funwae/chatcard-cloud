...
function validateIssuedAt(issuedAt: string): { valid: boolean; error?: string } {
...
cosignRouter.post('/:mh/cosign', requireAgentOrOwner, requireScope('proof:write'), async (req: AuthRequest, res) => {
  try {
    const { mh } = req.params;
    ...
    const authors: string[] = document?.claim?.authors ?? [];
    ...
    await prisma.proof.update({
      where: { multihash: mh },
      data: { document: updatedDocument as any },
    });
